...
import { processIntroduction, storeResultsSomehow, summaryIntroduction } from '../services/questionService.js'; // 가정된 서비스 파일
...
router.post('/staychatbot', async (req, res) => {
    ...
    try {
        ...
        // 세션에서 summaryIntroduction이 이미 실행됐는지 확인
        if (!req.session.summaryDone) {
            const summary = await summaryIntroduction(introduction);
            req.session.results = results;
            req.session.results.summary = summary;
            req.session.summaryDone = true; // 요약 진행 상태를 세션에 저장
        }
