...
drop table user, chat, question, tail;
...
CREATE TABLE user(
    ...
    INDEX (user_login_id)  -- user_id 컬럼에 인덱스 추가
) comment '유저 정보 저장';
...
CREATE TABLE chat (
    ...
    FOREIGN KEY (user_login_id) REFERENCES user(user_login_id)  -- 외래 키 제약 조건
) comment '왼쪽 사이드 채팅 바';
...
--     answer_text TEXT COMMENT '대분류 답변 저장',
--     answer_created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '생성 시각',
--     FOREIGN KEY (question_id) REFERENCES question(question_id)
-- ) comment '대질문 대답 저장';

CREATE TABLE extended_questions(
    tail_id INT AUTO_INCREMENT PRIMARY KEY COMMENT '무결성 컬럼',
    question_id INT COMMENT 'question 테이블의 question_id 참조',
    tail_question TEXT COMMENT '꼬리 질문 저장',
    tail_answer TEXT COMMENT '꼬리 질문의 대답',
    tail_tip VARCHAR(200) COMMENT '꼬리 질문 팁',
    tail_created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '생성 시각',
    FOREIGN KEY (question_id) REFERENCES initial_questions(question_id)
) COMMENT '꼬리질문 저장';


select * from user;
select * from chat;
select * from initial_questions;
-- select * from answer;
select * from extended_questions;

select chat_id from initial_questions where question_id = 10;
select * from initial_questions where chat_id=2;

-- 아래부터는 테스트 코드
insert into user(user_login_id, user_name, user_email, user_pass) values('tmddn3410','노승우','tmddn3410@naver.com',sha2('!Test1234',512));


insert into chat(user_login_id, introduction, chat_summary) values('tmddn3410', '
...
insert into question(chat_id,question_code,question_text,question_tip) values
...
-- (1, "대용량 데이터를 다루는 REST API를 개발할 때, 데이터베이스 쿼리 성능이 전체 응답 시간에 큰 영향을 미칩니다. 특히 복잡한 조인이나 대량 데이터에 대한 쿼리는 성능 저하의 주요 원인이 될 수 있습니다. 이를 해결하기 위해 다음과 같은 최적화를 진행했습니다
...
INSERT INTO extended_questions(question_id, tail_question, tail_answer, tail_tip) VALUES (?,?,?,?); -- <- 방법으로 함
...
-- 실제로는 로직상으로는 질문만 삽입후 대답은 update문으로 수정해야합니다.
INSERT INTO extended_questions(question_id, tail_question, tail_tip, tail_answer) 
values( 7, '꼬리질문이 들어갑니다 1', '꼬리질문 팁1','인덱스 추가, 쿼리 리팩토링, EXPLAIN PLAN 사용 등이 있습니다.');


-- 실제 로직 처럼 구현
-- 답변에 대한 질문 생성 사용자가 답변을 하면 다시 질문을 만드는 로직
-- 아직 답변을 안한상태
INSERT INTO extended_questions(question_id, tail_question, tail_tip, tail_answer) 
SELECT q.question_id, '꼬리질문이 들어갑니다 2', '꼬리질문 팁2',null
FROM question q
...
-- 사용자가 답변을 한 상태
UPDATE tail SET `tail_answer` = '꼬리 질문 답변 2' WHERE `tail_id` = '2';

-- 다시 질문
INSERT INTO tail(question_id, question_text, tail_question, tail_tip, tail_answer) 
SELECT q.question_id, q.question_text, '꼬리질문이 들어갑니다 3', '꼬리질문 팁3',null
FROM question q
WHERE q.question_id = 1;
